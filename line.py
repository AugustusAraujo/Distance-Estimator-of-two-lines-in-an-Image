...
def cal_new(slope_int, number_of_lines):
    if number_of_lines == 2:
        ...
        numerator = abs(
            (slope_int[1][1] + slope_int[0][1]) / 2
            - (slope_int[2][1] + slope_int[3][1]) / 2
        )
        denominator = math.sqrt(
            1
            + ((slope_int[0][0] + slope_int[1][0]) / 2)
            * ((slope_int[2][0] + slope_int[3][0]) / 2)
        )
        distance_pixel = numerator / denominator
        print(distance_pixel)
        distance = distance_pixel * float(get_ratio())
...
def cal_dst(line, number_of_lines):
    if number_of_lines == 2:
        ...
        numerator = abs(
            m * ((line[2][0]) - (line[0][0])) + ((line[0][1]) - (line[2][1]))
        )
        print(numerator)
        ...
        print(m)
        numerator = abs(
            m * ((line[4][0] + line[6][0]) / 2) - ((line[0][0] + line[2][0]) / 2)
        ) + (((line[0][1] + line[2][1]) / 2) - ((line[4][1] + line[6][1]) / 2))
        print(numerator)
        denominator = math.sqrt(1 + m * m)
        print(denominator)
        distance_pixel = numerator / denominator
...
def regression(dat, count_line):
    for k in range(len(dat)):
        ...
        if err <= err1:
            ...
            print(
                "max error",
                max_error * float(get_ratio()),
                "in the pixel",
                place * float(get_ratio()),
            )
    ...
    datacursor(
        display="multiple",
        arrowprops=dict(arrowstyle="simple", fc="green", alpha=0.5),
        draggable=True,
...
def find_line(img):
    ...
    cnts = cv2.findContours(edged2.copy(), cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
    cnts = cnts[0] if imutils.is_cv2() else cnts[1]
